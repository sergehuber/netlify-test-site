...
//_modules_dx-base-demo-components_javascript_custom_allStories.js_1540281359876.min.js
function activePageViewTab(){initializeActiveTab(".allStories","allStoriesTab","all")}function activateStoriesTabAjax(){$(".allStoriesTabItem").click(function(){var a=$(this).attr("view");var c=$(this).closest(".allStories");var b=updateQueryStringParameter(window.location.href,"allStoriesTab"+c.attr("uuid"),a);if(c.attr("disableAjax")=="true"){window.location.replace(b)}else{history.pushState(null,null,b);c.parent().load(c.attr("url")+".ajax?allStoriesTab"+c.attr("uuid")+"="+a+" #"+c.attr("id"),function(){activateStoriesTabAjax();activePageViewTab()})}})}$(document).ready(function(){activePageViewTab();activateStoriesTabAjax()});;
